...
const init = () => {
  ...
  //우주 공간 만들기
  const universeMap = new THREE.TextureLoader().load('./image/universe.jpg');
  const material_universe = new THREE.MeshBasicMaterial({
    map: universeMap,
    side: THREE.BackSide,
  });
  const geometry_univ = new THREE.SphereGeometry(500, 32, 32);

  universe = new THREE.Mesh(geometry_univ, material_universe);
  scene.add(universe);
